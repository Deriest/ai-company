...
may not be absolute, may not normalize outside the root via ``..``, may not
abuse a sibling-prefix bypass (``/root2/...`` when the root is ``/root``), and
may not escape through a symlink inside the workspace that points outside.
"""
import os
...
def resolve_workspace_path(workspace_root: str, path: str) -> str:
    ...
    - Resolves symlinks (os.path.realpath) and re-verifies containment, so a
      symlink inside the workspace pointing outside is blocked
