...
**Authentication:** JWT Bearer Token (optional — desktop app binds to loopback; `POST /auth/login` + `GET /auth/me`)
...
The credential is a per-install random secret generated by the Electron main
process into `userData/aic-ade/identity.json` (mode 0600) and passed to the
backend via `AIC_IDENTITY_FILE`. When neither the file nor the env var is set
(standalone/dev/tests), the fallback is `admin` / `admin123`.

**Request:**
```json
{
  "username": "admin",
  "password": "<per-install random hex from identity.json or admin123 fallback>"
}
```
